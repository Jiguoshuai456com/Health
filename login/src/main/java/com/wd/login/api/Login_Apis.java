 ...
 * */public interface Login_Apis {
    ...
    @FormUrlEncoded
    Observable<login_ResetUserPwdBean>doResetUserPwd(@Field("email") String email,@Field("pwd1") String pwd1,@Field("pwd2") String pwd2);
    //病友圈列表展示
    @GET("doctor/sickCircle/v1/findSickCircleList")
    Observable<login_SinckCircleBean>doSinckCircle(@Query("departmentId")int departmentId,@Query("page")int page,@Query("count")int count);
}
